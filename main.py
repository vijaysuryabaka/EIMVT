...
def extract_meaning(text,to_lang):
    """
...
def txtxt(text, tolang):
    translate_client = translate.Client()

    # Translates the text into the target language
    translated_text = translate_client.translate(text, target_language=tolang)
    text = html.unescape(translated_text['translatedText'])
    client = tts.TextToSpeechClient()
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    local_filename = f"ttt_{timestamp}.mp3"
    local_filepath = os.path.join("static/extra_op", local_filename)
    # Use a temp directory for local storage

    # Save locally
    if not os.path.exists("static/extra_op"):
        os.makedirs("static/extra_op")
    with open(local_filepath, "wb") as out_file:
        out_file.write(response.audio_content)
        print(f"Audio content written to {local_filepath}")

    return text, local_filepath
...
@app.route('/conctrans', methods=['POST'])
def conctrans():
    ...
    meaning = extract_meaning(text, to_lang)
    print(meaning)
    meaningb = meaning.replace('**', '').replace('**', '')
    print(meaningb)
    return jsonify({"success": True,
                   "translation":meaningb})

@app.route('/txt2txt', methods = ['POST'])
def txt2txt():
    f_lang = request.form['from']
    t_lang = request.form['to']
    text = request.form['text']
    from_lang = language_mapping.get(f_lang)
    to_lang = language_mapping.get(t_lang)
    print("ttt",from_lang, to_lang)
    transtext, audio = txtxt(text,to_lang)
    print(audio)
    return jsonify({'success': True,
                    "text": transtext,
                    "audio": audio})
